...
$main_menus = [
  ...
  'about_us' => [
	...
	'submenu' => [
		...
		'layanan' => [
			'text' => 'Layanan',
			'url' => 'index.php?p=layanan',
		],
		'staf_perpustakaan' => [
			'text' => 'Staf Perpustakaan',
			'url' => 'index.php?p=librarian',
		],
	],
  ],
...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top">
        ...
        <ul class="navbar-nav ml-auto">
          ...
          foreach ($main_menus as $key => $main_menu) {
			...
			if(count($main_menu['submenu'])>0){
				foreach($main_menu['submenu'] as $key2=>$submenu){
					$active_dropdown = '';
            		if (isset($_GET['p'])) {
						if($key === 'about_us' && ($_GET['p'] === 'profil' || $_GET['p'] === 'fasilitas' || $_GET['p'] === 'layanan' || $_GET['p'] === 'librarian')){
							$active_dropdown = 'active';
						}
						elseif($key === 'masukan' && ($_GET['p'] === 'usul-buku' || $_GET['p'] === 'kotak-saran')){
							$active_dropdown = 'active';
						}
					}
				}  
				...
				$menu_str = '';
				$menu_str .= '<li class="nav-item dropdown '.$active_dropdown.'">
                  <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown"
                     aria-haspopup="true" aria-expanded="false">'.$main_menu['text'].'</a>
                  <div class="dropdown-menu dropdown-menu-right">';
				foreach($main_menu['submenu'] as $submenu){
					$menu_str .= '<a class="dropdown-item" href="'.$submenu['url'].'">'.$submenu['text'].'</a>';
				}  
				$menu_str .= '</div></li>';
			}
			else{
	...
	HTML;
          ...
          $menu_admin_active = isset($_GET['p']) && $_GET['p'] === 'login' ? 'active' : '';
          if ($is_login) {
            ?>
          ...
          <?php } else { ?>
              <li class="nav-item <?= $menu_member_active; ?>">
                  <a class="nav-link btn btn-primary" href="index.php?p=member"><?= __('Member Area') ?></a>
              </li>
              <li class="nav-item ml-2 <?= $menu_admin_active; ?>">
                  <a class="nav-link btn btn-primary" href="index.php?p=login">Area Admin</a>
              </li>
          <?php } ?>
			<!--
...
<style type="text/css">
  ...
  .navbar .btn-primary {transition: .2s; color: #fff!important}
  .navbar.scrolled {background: rgb(0, 105, 217);}
  .navbar.scrolled .btn-primary {
      color: #000!important;
      background-color: #FFF404;
      border-color: #FFF404;}
  .navbar.scrolled .btn-primary:hover, .navbar.scrolled .btn-primary:active {
      color: rgb(0, 0, 0);
      background-color: rgba(255, 244, 0, 0.9);
      border-color: #FFF404;}
  .navbar.scrolled.navbar-dark .navbar-nav .nav-link.btn-primary:focus, .navbar.scrolled.navbar-dark .navbar-nav .nav-link.btn-primary:hover {color: rgba(0, 0, 0, 1);}
  .navbar.scrolled .nav-item.active .btn-primary {color: #000!important}
  .navbar-dark .navbar-nav .nav-link{color: rgba(255,255,255,0.8)}
  .navbar-dark .navbar-nav .active>.nav-link, .navbar-dark .navbar-nav .nav-link.active, .navbar-dark .navbar-nav .nav-link.show, .navbar-dark .navbar-nav .show>.nav-link {font-weight: bold}
